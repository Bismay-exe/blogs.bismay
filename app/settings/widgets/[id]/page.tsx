...
const TOKENS = [
    { label: '{{title}}', desc: 'Article Title' },
    { label: '{{slug}}', desc: 'Article Slug' },
    { label: '{{category}}', desc: 'Category' },
    { label: '{{readingTime}}', desc: 'Reading Time' },
    { label: '{{date}}', desc: 'Date' },
    { label: '{{url}}', desc: 'Site URL' },
]
...
export default function WidgetDetailPage() {
    ...
    const { items, updateWidgetConfig, addWidget, deleteWidget, toggleWidgetActive } = useWidgetsSettings()

    const isNewMode = rawId === 'new'
    const existingWidget = items.find((w) => w.id === rawId)

    // Form States
    const [type, setType] = useState<WidgetType>('customHtml')
    ...
    useEffect(() => {
        if (isNewMode) {
            setType('customHtml')
            setTitle('Custom Sidebar Card')
            setCustomTitle('Announcement')
            setHtml(SNIPPETS[0].html)
            setCss('')
            setJs('')
            setMarkdown('')
            setEnabled(true)
            ...
            setCustomTitle(existingWidget.config?.customTitle ?? existingWidget.title)
            setHtml(existingWidget.config?.html || '')
            setCss(existingWidget.config?.css || '')
            setJs(existingWidget.config?.js || '')
            setMarkdown(existingWidget.config?.markdown || '')
            setEnabled(existingWidget.enabled)
            setActiveTab(existingWidget.type === 'customMarkdown' ? 'markdown' : 'html')
        }
    }, [isNewMode, existingWidget, rawId])
    ...
    const handleSave = () => {
        if (isNewMode) {
            ...
            setTimeout(() => {
                router.push(`/settings/widgets/${newId}`)
            }, 500)
        } else if (existingWidget) {
            updateWidgetConfig(existingWidget.id, {
                title: title.trim() || existingWidget.title,
                enabled,
                config: {
    ...
    const handleDelete = () => {
        ...
        if (confirm(`Are you sure you want to delete "${existingWidget.title}"?`)) {
            deleteWidget(existingWidget.id)
            router.push('/settings/widgets')
        }
    }

    const insertToken = (token: string) => {
        if (type === 'customMarkdown' || activeTab === 'markdown') {
    ...
    const isCustomCode = type === 'customHtml'
    const isMarkdown = type === 'customMarkdown'

    // Transient widget object for previewing
    const previewWidgetInstance: WidgetInstance = {
        id: isNewMode ? 'preview-new' : existingWidget?.id || rawId,
        type,
        title,
        enabled: true,
        isBuiltIn: !isNewMode && (existingWidget?.isBuiltIn ?? false),
        config: {
            customTitle,
            html,
            css,
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <Link
                        ...
                        className="p-2.5 rounded-2xl border border-sec/20 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-sec hover:text-fg transition-colors"
                        title="Back to Widgets Hub"
                    >
                        <ArrowLeft size={16} />
                    </Link>
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <span className="text-[11px] font-mono uppercase text-accent font-bold">
                                {isNewMode ? 'Create New' : existingWidget?.isBuiltIn ? 'Built-in Widget' : 'Custom Widget'}
                            </span>
                            <span className="text-sec/40 text-xs">/</span>
                            <span className="text-xs font-mono text-sec">
                                {isNewMode ? 'New' : rawId}
                            </span>
                        </div>
                        <h1 className="text-xl sm:text-2xl font-extrabold text-fg tracking-tight">
                            {isNewMode ? 'Create Custom Widget' : title || 'Widget Settings'}
                        </h1>
                    </div>
                </div>
                ...
                {/* Top Action Buttons */}
                <div className="flex items-center gap-2">
                    {!isNewMode && existingWidget && !existingWidget.isBuiltIn && (
                        <button
                            type="button"
                    ...
                    <button
                        type="button"
                        onClick={handleSave}
                        className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-accent text-white dark:text-[#0C0C0C] font-mono text-xs font-bold shadow-xs hover:opacity-90 transition-opacity cursor-pointer"
                    >
                        {saveStatus ? <Check size={14} /> : <Save size={14} />}
                        <span>{saveStatus || (isNewMode ? 'Publish Widget' : 'Save Changes')}</span>
                    </button>
                </div>
            </div>

            {/* Main Configuration Grid: Left Form Controls, Right Live Preview */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                {/* Left Form Zone (7 cols) */}
                <div className="lg:col-span-7 space-y-5">
                    ...
                    <div className="p-5 sm:p-6 rounded-3xl border border-sec/20 bg-black/2 dark:bg-white/2 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xs font-mono font-bold uppercase tracking-wider text-fg">
                                General Settings
                            </h2>
                            <label className="flex items-center gap-2 cursor-pointer">
                                <span className="text-xs font-mono text-sec">Sidebar Visibility:</span>
                                <input
                                    type="checkbox"
                                    checked={enabled}
                                ...
                                />
                                <span className={`text-xs font-bold ${enabled ? 'text-emerald-400' : 'text-sec'}`}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-xs font-mono text-sec">Widget Label (Admin Title)</label>
                                <input
                                    type="text"
                                    value={title}
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-mono text-sec">Container Heading (Display Title)</label>
                                <input
                                    type="text"
                                    value={customTitle}
                                    onChange={(e) => setCustomTitle(e.target.value)}
                                    placeholder="Leave empty to hide card header"
                                    className="w-full px-3.5 py-2.5 rounded-xl bg-black/5 dark:bg-white/5 border border-sec/20 text-xs font-medium text-fg focus:border-accent outline-none"
                                />
                            </div>
                        </div>

                        {/* Widget Type Selector for New Mode */}
                        {isNewMode && (
                            <div className="space-y-1.5 pt-2 border-t border-sec/10">
                                <label className="text-xs font-mono text-sec">Widget Technology Type</label>
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        type="button"
                                        onClick={() => setType('customHtml')}
                                        className={`p-3 rounded-xl border flex items-center gap-2.5 text-left cursor-pointer transition-colors ${type === 'customHtml'
                                            ? 'border-accent bg-accent/10 text-fg'
                                            : 'border-sec/15 bg-black/3 dark:bg-white/3 text-sec hover:text-fg'
                                            }`}
                                    >
                                        <Code2 size={18} className="text-accent" />
                                        <div>
                                            <p className="text-xs font-bold">Custom HTML / CSS / JS</p>
                                            <p className="text-[10px] text-sec">Embed custom code and dynamic tokens</p>
                                        </div>
                                    </button>

                                    <button
                                        type="button"
                                        onClick={() => setType('customMarkdown')}
                                        className={`p-3 rounded-xl border flex items-center gap-2.5 text-left cursor-pointer transition-colors ${type === 'customMarkdown'
                                            ? 'border-accent bg-accent/10 text-fg'
                                            : 'border-sec/15 bg-black/3 dark:bg-white/3 text-sec hover:text-fg'
                                            }`}
                                    >
                                        ...
                                        <div>
                                            <p className="text-xs font-bold">Markdown Callout Card</p>
                                            <p className="text-[10px] text-sec">Rich markdown text and callout styling</p>
                                        </div>
                                    </button>
                                </div>
                            </div>
                    ...
                    {/* Card 2: Code Editor for Custom HTML/JS/CSS */}
                    {isCustomCode && (
                        <div className="p-5 sm:p-6 rounded-3xl border border-sec/20 bg-black/2 dark:bg-white/2 space-y-4">
                                ...
                                <div className="flex items-center gap-1.5 bg-black/5 dark:bg-white/5 p-1 rounded-xl border border-sec/10">
                                    {(['html', 'css', 'js'] as const).map((tab) => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold uppercase transition-colors cursor-pointer ${activeTab === tab
                            ...
                            {/* Dynamic Tokens & Theme Colors Cheat Sheet */}
                            ...
                            {activeTab === 'html' && (
                                <div className="space-y-1.5">
                                    <span className="text-[11px] font-mono text-sec">HTML Markup</span>
                                    <textarea
                                        rows={12}
                                        value={html}
                                        onChange={(e) => setHtml(e.target.value)}
                                        placeholder="<div>Your HTML here...</div>"
                                        className="w-full p-4 rounded-2xl bg-[#101012] text-[#E4E4E7] font-mono text-xs leading-relaxed border border-sec/20 focus:border-accent outline-none font-medium"
                                        spellCheck={false}
                                    />
                                </div>
                            )}
                            ...
                            {activeTab === 'css' && (
                                <div className="space-y-1.5">
                                    <span className="text-[11px] font-mono text-sec">Scoped CSS Rules</span>
                                    <textarea
                                        rows={12}
                                        value={css}
                            ...
                            {activeTab === 'js' && (
                                <div className="space-y-1.5">
                                    <span className="text-[11px] font-mono text-sec">Client JavaScript</span>
                                    <textarea
                                        rows={12}
                                        value={js}
                                        onChange={(e) => setJs(e.target.value)}
                                        placeholder="// container is the root DOM element\n// console.log('Widget mounted', container, articleData);"
                                        className="w-full p-4 rounded-2xl bg-[#101012] text-[#E4E4E7] font-mono text-xs leading-relaxed border border-sec/20 focus:border-accent outline-none font-medium"
                                        spellCheck={false}
                                    />
                                </div>
                    ...
                    {!isCustomCode && !isMarkdown && (
                        <div className="p-6 rounded-3xl border border-sec/20 bg-black/2 dark:bg-white/2 space-y-3">
                            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-fg">
                                Native React Component Information
                            </h3>
                            <p className="text-xs text-sec leading-relaxed">
                                This is a built-in React widget with native styling, responsive design, and dynamic theme tokens. You can adjust its container display title above or reorder where it appears in the sidebar via the Reader Studio.
                            </p>
                        </div>
                    )}
                </div>

                {/* Right Live Preview Zone (5 cols) */}
                <div className="lg:col-span-5 space-y-4 sticky top-6">
                    <div className="p-5 sm:p-6 rounded-3xl border border-sec/20 bg-black/2 dark:bg-white/2 space-y-4 shadow-xs">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl border border-sec/20 text-[11px] font-mono text-sec hover:text-fg transition-colors cursor-pointer"
                            >
                                {previewTheme === 'dark' ? (
                                    <>
                    ...
                    {/* Simulated Article Sidebar Card Viewport */}
                    <div
                        className={`p-6 rounded-3xl border border-sec/20 overflow-hidden transition-colors ${previewTheme === 'dark' ? 'bg-[#0C0C0C] text-[#E7E7E7]' : 'bg-[#FFFFFF] text-[#191919]'
                            }`}
                    >
                        <WidgetErrorBoundary widgetTitle={title}>
                            {renderWidgetComponent(previewWidgetInstance, SAMPLE_ARTICLE, previewTheme)}
                        </WidgetErrorBoundary>
                    </div>

                    <p className="text-[10px] font-mono text-sec text-center">
                        Simulated with sample article metadata
                    </p>
                </div>
            </div>
        </div>
