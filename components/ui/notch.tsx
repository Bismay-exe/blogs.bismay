...
export interface NotchProps {
  ...
  items: NotchItem[];
  /** Layout mode: 'fixed' (viewport floating) or 'inline' (within content/card flow). */
  variant?: "fixed" | "inline";
  /** Pin the notch to the top or bottom of the viewport. */
  position?: "top" | "bottom";
  /** Horizontal alignment of the notch. */
...
function NotchDivider() {
  return (
    <span
      aria-hidden
      className="mx-0.5 h-5 w-px shrink-0 self-center opacity-35"
      style={{
        backgroundImage:
...
export const Notch = ({
  ...
  const getSelectedId = (item: NotchItem) =>
    item.value !== undefined
      ? item.value
      : (internalSelected[item.id] ?? item.options[0]?.id);

  const getSelectedOption = (item: NotchItem) => {
    const selId = getSelectedId(item);
    return item.options.find((o) => o.id === selId);
  };
        ...
        {openItem.options.map((option) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                "flex w-full items-center justify-between gap-6 rounded-md px-3 py-2 text-left text-xs font-medium whitespace-nowrap transition-colors cursor-pointer",
                active
                  ? "text-fg font-semibold"
                  : "text-sec hover:bg-fg/5 hover:text-fg",
              )}
              style={
                active
                  ? {
                      background: `color-mix(in oklab, ${accentColor} 20%, var(--background))`,
                      boxShadow: `inset 0 0 0 1px color-mix(in oklab, ${accentColor} 45%, transparent)`,
                    }
                  : undefined
              }
            >
        ...
        return (
          <React.Fragment key={item.id}>
            <button
              ...
              className={cn(
                "group flex items-center gap-2 rounded-full px-3 py-2 text-xs font-medium whitespace-nowrap text-sec transition-colors hover:bg-fg/5 hover:text-fg cursor-pointer",
                itemClassName,
              )}
            >
              {item.icon ? (
              ...
              {(item.showValue ?? showSelectedValue) && selected ? (
                <span className="text-sec font-mono text-[11px]">{selected.label}</span>
              ) : null}
            </button>
  ...
  const shellNode = (
    <motion.div
      ...
      className={cn(
        "pointer-events-auto flex w-fit flex-col overflow-hidden rounded-xl border border-sec/15 bg-bg/95 shadow-[0_12px_40px_-8px_rgba(0,0,0,0.35)] ring-1 ring-sec/10 backdrop-blur-2xl ring-inset",
        className,
      )}
    >
  ...
  if (variant === "inline") {
    return (
      <div className={cn("relative flex w-full", alignClass)}>
        <div className="relative inline-block">
          {/* Static placeholder to keep layout completely static and prevent any shifts in content below */}
          <div className="invisible pointer-events-none flex items-center gap-1 p-1">
            {items.map((item, index) => {
              const selected = getSelectedOption(item);
              const isLast = index === items.length - 1;
              return (
                <React.Fragment key={`placeholder_${item.id}`}>
                  <div className="flex items-center gap-2 rounded-full px-3 py-2 text-xs font-medium whitespace-nowrap">
                    {item.icon ? <span>{item.icon}</span> : null}
                    <span>{item.label}</span>
                    {selected ? <span className="text-[11px]">{selected.label}</span> : null}
                  </div>
                  {showDividers && !isLast ? <NotchDivider /> : null}
                </React.Fragment>
              );
          ...
          {/* Animated floating shell that expands on top without moving anything below */}
