...
import { createTickPlayer } from "@/lib/ui/tick-sound";
import {
    capturePointer,
    releasePointer,
    TOUCH_GESTURE_CLASS,
...
export function WheelPicker({
    ...
    if (reduce) {
        ...
        return (
            <div
                className={cn(
                    "relative overflow-hidden rounded-2xl border border-sec bg-bg",
            ...
            >
                <div
                    className="pointer-events-none absolute inset-x-0 top-1/2 z-10 -translate-y-1/2 border-sec/30 border-y bg-fg/4"
                    style={{ height: itemHeight }}
                />
                <ul
                    className={cn(
                        "h-full snap-y snap-mandatory overflow-y-auto scroll-smooth [-ms-overflow-style:none] scrollbar-none [&::-webkit-scrollbar]:hidden",
                        maskFade,
                    )}
    ...
    return (
        <div
            ...
            className={cn(
                "relative touch-none overflow-hidden rounded-2xl border border-sec/30 bg-card outline-none focus-visible:ring-2 focus-visible:ring-foreground/20",
        ...
        >
            ...
            >
                {options.map((option, i) => (
                    <li
                        ...
                        data-index={i}
                        className="absolute inset-x-0 flex items-center justify-center font-medium text-sec"
                        style={{
                            top: -itemHeight / 2,
                            height: itemHeight,
          ...
          drum's projection so the two copies register exactly. */}
            <div
                className="pointer-events-none absolute inset-x-0 top-1/2 z-10 -translate-y-1/2 overflow-hidden rounded-md bg-fg/4"
                ...
                >
                    {options.map((option, i) => (
                        <li
                            key={optionValue(option)}
                            data-index={i}
                            className="absolute inset-x-0 flex items-center justify-center font-medium text-fg"
                            style={{
                                top: -itemHeight / 2,
                                height: itemHeight,
                                transform: `rotateX(${-itemAngle * i}deg) translateZ(${radius}px)`,
